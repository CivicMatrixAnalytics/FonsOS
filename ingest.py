...
def run_fonsos_ingest():
    start_date = datetime.date(2026, 5, 10)
    today = datetime.date.today()
    print(f"[*] FonsOS Ingestion Engine Started: Range [{start_date} to {today}]")
    ...
    for q in search_queries:
        ...
        for item in feed.entries:
            ...
            try:
                ...
                inc_date = start_date.strftime("%Y-%m-%d")
        ...
        if batch:
            try:
                res = supabase.table("incidents").upsert(batch, on_conflict="proof_url").execute()
                count = len(batch)
                total_inserted += count
                print(f"[✓] Query '{q}' -> Synced {count} records.")
            except Exception as e:
                print(f"[!] Error syncing batch for query '{q}': {e}")

        time.sleep(1.2)

    print(f"\n[★] FonsOS Ingestion Complete! Total Processed: {total_inserted} incidents.")
